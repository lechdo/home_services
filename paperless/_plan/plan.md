...
- **Pas encore fait** : décision et exécution de la migration des données réelles de l'ancienne instance (volumes `pgdata`/`media`/`redisdata` + `rclone.conf`, cf. point 4 ci-dessus) vers cette instance neuve si on veut continuer avec l'historique existant plutôt que repartir de zéro ; bascule physique effective ; binding du port sur l'IP LAN (actuellement encore `127.0.0.1`, cf. point 1) ; mise à jour de l'IP dans `edge/nginx/conf.d/paperless.conf` ; test réel serveur éteint/rallumé.

...
## Phase 1 — Synchronisation Google Drive → Paperless — **implémentée et validée (2026-08-14)**

- **Fait** : sidecar `sidecar-gdrive-sync` ajouté à `compose.yaml` (image Alpine + rclone, depuis `GDRIVE_REMOTE_PATH` vers le dossier `consume` de Paperless, toutes les 5 minutes). Dossier Drive dédié retenu : `controlled_chaos`.
- **Fait** : accès en **lecture seule** (`scope = drive.readonly` côté rclone) plutôt qu'accès complet — ce service ne peut structurellement pas écrire sur le Drive, cohérent avec la contrainte d'immutabilité de la source. Scripts `scripts/authorize-gdrive.sh` (OAuth, indépendant de celui de bitwarden) et `scripts/setup-gdrive-sync.sh` (vérification d'accès + activation, idempotent).
- **Bug découvert à l'usage (2026-08-14)** : l'hypothèse initiale ("`rclone copy` sans tracking custom, un fichier déjà ingéré et recopié est rejeté par la détection de doublon par hash native à Paperless-ngx") s'est révélée **fausse en pratique** — des documents identiques (même nom de fichier, fichiers déjà binaires PDF/JPG, pas de conversion Google Docs en jeu) apparaissaient réellement dupliqués dans l'UI Paperless, pas juste retentés sans effet. Cause : `rclone copy` seul n'a aucune mémoire de ce qu'il a déjà envoyé une fois que Paperless retire le fichier de `consume/` après ingestion — au cycle suivant (toutes les 5 min), le fichier est toujours présent sur Drive (immuable) et absent de `consume/`, donc recopié, et Paperless en crée un nouveau document à chaque fois.
- **Fix (2026-08-14)** : ledger persistant (`/state/synced.tsv`, nouveau volume Docker `gdrive-sync-state`, jamais vidé par Paperless) mémorisant chemin+taille+date de modification de chaque fichier déjà envoyé une fois (`rclone lsf --format pst`, comparé ligne à ligne). Un fichier réapparaissant à l'identique n'est plus jamais recopié ; une modification réelle du fichier source (taille/date différentes) reste resynchronisée normalement. Toujours `copy`/`copyto`, jamais `sync`, immutabilité de la source inchangée.
- **Décision opérationnelle (2026-08-14)** : plutôt que de dédupliquer a posteriori les documents déjà créés en double, reset complet côté Paperless (`docker compose down -v` sur le second serveur — pgdata/media/redisdata/data ET le ledger repartent à zéro ensemble, états cohérents), re-provisioning de la taxonomie (`provisioning/apply.py`), puis réactivation du sync corrigé qui redigère tout `controlled_chaos` proprement en une seule fois. Cohérent avec la propriété de reconstructibilité déjà documentée (`plan-sauvegarde.md`) : rien n'est perdu, Drive reste la source de vérité.
- **Premier reset contaminé, refait proprement** : le tout premier `down -v` a été exécuté pendant que l'ancien sidecar (pré-fix) tournait encore sur le serveur (le `git pull` n'avait rien de nouveau à récupérer — les commits n'étaient que locaux sur le laptop, non poussés vers `origin`) : la ré-ingestion qui a suivi est repartie d'une base neuve mais avec l'ancien mécanisme buggé encore actif, produisant ~140 documents pour 101 fichiers Drive. Une fois `origin/main` à jour et l'ancien conteneur supprimé manuellement, un second `down -v` + re-ingestion complète avec le code corrigé a donné **91 documents pour 101 fichiers Drive** — écart explicable par de vrais doublons de contenu sur Drive lui-même (ex. `Dossier.MDPH_V.VINCE_page 9à16.pdf` présent plusieurs fois avec un contenu identique, correctement absorbés par la détection de doublon par hash de Paperless-ngx, qui s'avère donc fonctionner correctement une fois que le sync ne lui présente plus de fausses "nouvelles versions").
- **Fichiers same-name-different-content, pas un bug** : découvert au passage que Google Drive autorise plusieurs fichiers distincts portant le même nom dans un dossier (contrairement à un système de fichiers classique) — ex. trois fichiers `facture.pdf` de tailles différentes, correctement ingérés comme trois documents séparés par Paperless. Peut prêter à confusion dans l'UI Paperless (même titre affiché pour des documents réellement différents) mais n'est pas un défaut du sync ou de la dédup.
- **Objectif de sortie de phase validé (2026-08-14)** : test réel en conditions de production — un nouveau fichier déposé dans `controlled_chaos` est apparu automatiquement dans Paperless (OCR fait), sans intervention manuelle, sans duplication.
...
## Phase 9 — Migration vers un second serveur dédié — **en cours (2026-08-14)**
...
- **Ce qui change** :
  ...
  3. **Déploiement du code : `git clone` du monorepo sur le second serveur**, plutôt qu'un `rsync` du dossier `paperless/` — décision prise le 2026-08-14. Ramène `compose.yaml`, `provisioning/`, `rad-lad/`, `scripts/`, `_plan/`, et **`.env`** (suivi par git dans ce repo, contrairement à ce qu'une version antérieure de cette phase supposait). Nécessite un accès git authentifié (clé SSH/PAT) configuré sur le second serveur, ce que `rsync` n'exigeait pas. Le clone ramène l'intégralité du monorepo (tous les services), pas seulement `paperless/` — sans conséquence puisque les services sont autonomes (cf. `CLAUDE.md` racine) et que seul `paperless/compose.yaml` est lancé sur ce serveur ; un sparse-checkout limité à `paperless/` reste possible si on veut l'éviter.
  4. **Ce que le clone ne couvre pas — toujours une copie manuelle** :
     - `rclone.conf` : seul `rclone.conf.example` est suivi par git ; le vrai fichier (token OAuth) doit être copié manuellement (scp/rsync) vers le second serveur, sinon la sync Drive échoue silencieusement à l'authentification.
     - Les volumes Docker nommés `pgdata`, `media`, `redisdata` : ce sont des volumes gérés par Docker, pas des bind mounts vers le repo — hors du champ de git par nature, clone ou rsync ne change rien ici. Export/copie manuelle (ex. `docker run --rm -v <vol>:/data -v $(pwd):/backup alpine tar czf /backup/<vol>.tgz /data`), pas de script de migration dédié à écrire — ces données sont de toute façon reconstructibles depuis Google Drive si besoin (cf. `plan-sauvegarde.md`), donc pas de procédure filet de sécurité particulière au-delà de la copie elle-même.
  5. `sidecar-gdrive-sync` (rclone) suit paperless sur le second serveur — aucune dépendance à la machine d'`edge`, aucun changement de logique de synchronisation.
...
- **Fait et validé (2026-08-14)** : `git clone` du monorepo sur le second serveur, puis mise en place locale d'une **instance neuve** (pas une restauration des volumes de l'ancienne — choix explicite pour valider rapidement le socle sur ce serveur avant de migrer les données réelles) : `mkdir -p export consume`, `docker compose up -d` (sans le profil `gdrive-sync`, `rclone.conf` n'étant pas encore copié), webserver `healthy`, page de connexion accessible en local (`127.0.0.1:8082`), taxonomie (types/tags/custom fields/workflow RAD/LAD) re-provisionnée via `provisioning/apply.py` sur cette base vierge. `.env` récupéré tel quel via le clone (suivi par git dans ce repo) — pas de régénération de secrets nécessaire, `pgdata` étant vide sur ce serveur.
- **Fait et validé (2026-08-14)** : synchronisation Google Drive activée sur cette instance (`rclone.conf` généré via `authorize-gdrive.sh` puis copié par `rsync` depuis le laptop, `setup-gdrive-sync.sh` exécuté) — ingestion en cours du dossier `controlled_chaos` au moment de la rédaction (premier lot de documents réels).
  - **Bug corrigé dans `authorize-gdrive.sh`** : le `docker run rclone/rclone authorize` ne publiait pas le port local (`53682`) sur lequel `rclone authorize` sert sa page de redirection intermédiaire vers Google — ce `127.0.0.1:53682` était celui du conteneur, injoignable depuis le navigateur de l'hôte. Un `-p 53682:53682` ne suffit pas non plus (Docker route le trafic publié vers l'interface externe du conteneur, jamais vers son loopback interne, alors que `rclone authorize` s'y bind spécifiquement). Corrigé avec `--network host` (Linux uniquement) : le conteneur partage alors directement la pile réseau de l'hôte, donc son `127.0.0.1` est bien celui de la machine qui lance le script.
- **Fait (2026-08-14)** : IP LAN du second serveur connue et appliquée : **`192.168.1.109`**. `compose.yaml` mis à jour (`192.168.1.109:8082:8000`, remplace le binding `127.0.0.1`) ; `edge/nginx/conf.d/paperless.conf` basculé vers cet upstream (voir `edge/_plan/plan.md` phase 7 pour le détail du routage, des timeouts courts et de la page d'indisponibilité).
- **Décision explicite (2026-08-14) — pare-feu reporté** : aucun pare-feu actif sur le second serveur (`ufw` présent mais inactif, pas d'iptables configuré). Activer un pare-feu pour la première fois sur une machine administrée à distance est une décision à part (risque de coupure de l'accès SSH si mal fait) — reportée plutôt que faite dans la précipitation. En attendant, le port `8082/tcp` reste ouvert à tout le réseau local (réseau domestique de confiance, risque jugé acceptable temporairement) plutôt que restreint à la seule IP d'`edge` (`192.168.1.99`, connue). À faire avant de considérer la Phase 9 terminée : activer `ufw` avec la séquence sûre (autoriser SSH explicitement d'abord, puis `ufw allow from 192.168.1.99 to any port 8082 proto tcp`, policy par défaut `deny incoming`, puis activation).
- **Pas encore fait** : décision et exécution de la migration des données de l'ancienne instance (volumes `pgdata`/`media`/`redisdata` de l'instance d'origine, cf. point 4 ci-dessus) vers cette instance neuve si on veut continuer avec l'historique existant plutôt que repartir de zéro (pour l'instant, deux instances Paperless distinctes coexistent, seule celle du second serveur est routée par edge) ; pare-feu (voir ci-dessus) ; test réel serveur éteint/rallumé (page 503 d'edge, reprise sans redémarrer edge).

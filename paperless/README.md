...
**Dernier état (2026-08-12)** : socle Docker Compose implémenté et validé (Phase 0 partielle), exposition Internet via `edge` faite (Phase 8), synchronisation Google Drive préparée mais pas activée (Phase 1), modélisation Custom Fields faite et validée (Phase 2), squelette RAD/LAD + webhook Paperless implémentés et validés de bout en bout (Phase 3) — la classification/extraction elle-même (Phase 4+) n'est pas encore développée, et aucun document réel (hors tests) n'a encore été traité.

...
| **minecraft** | Non applicable — pas de config structurelle créée via API, tout est dans `compose.yaml`/`.env` (dossier `maps/` mis à part, qui est une donnée utilisateur, pas de la config) | Écrit (`_plan/plan-sauvegarde.md`), **non implémenté (2026-08-17)** : plan posé (sauvegarde de `maps/`), outil et fréquence à définir à l'implémentation, restauration à tester avant de considérer la phase faite |

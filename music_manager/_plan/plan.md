...
Validation initiale faite dans un environnement de développement local (containers de test, détruits ensuite) — voir Phase 4 ci-dessous pour la validation réelle, en production sur les deux machines cibles.

## Phase 4 — déployée et testée en conditions réelles sur les deux machines cibles (2026-08-22)

Contrairement à une première tentative de validation locale dans l'environnement de développement (containers `edge` lancés directement là, sans rapport avec la vraie machine de production — **erreur signalée par l'utilisateur, corrigée** : jamais de service tournant en continu ailleurs que sur les machines cibles réelles), le déploiement réel a été fait par le chemin standard de ce dépôt : commit + push, puis `rsync`/`ssh` vers les deux machines.

- **`edge` (Raspberry Pi, `raspi-home.local`)** : `edge/nginx/conf.d/music.conf` et `edge/.env` (`music-jvince` ajouté à `DUCKDNS_SUBDOMAINS`, liste A+AAAA classique — contrairement à `minecraft-jvince`, un navigateur/Symfonium font du Happy Eyeballs) rsyncés vers `~/home_services/edge`. `sidecar-ddns` recréé → `OK` confirmé, sans régression sur les 3 sous-domaines déjà en place. Certificat **staging** émis et vérifié depuis le Pi lui-même, puis **production** (`--issue --server letsencrypt --force`), installé, vérifié : `issuer=C=US, O=Let's Encrypt, CN=YE2` (sans `STAGING`), valide jusqu'au 2026-11-20. `nginx -t && nginx -s reload` : succès, **aucune régression** sur bitwarden/paperless/vikunja (re-testés immédiatement après, `200`/`302` comme attendu).
- **`music_manager` (second serveur physique, `192.168.1.109`)** : premier déploiement de ce service, jamais fait avant cette phase. `rsync` du dossier, `.env` créé avec un `FETCHER_SECRET_KEY` généré (`openssl rand -hex 32`), `docker compose up -d --build` : les deux conteneurs démarrent, publiés sur `192.168.1.109:8087`/`8088` comme prévu.
- **Bug réel trouvé en testant `/fetcher` via edge, absent des tests locaux (qui n'avaient testé fetcher qu'en accès direct sans préfixe)** : `404` sur `/fetcher/login`. Cause : `location /fetcher/ { proxy_pass http://...:8088; }` (sans slash final) transmet le chemin complet `/fetcher/login` à Flask, qui ne route que `/login` — `ProxyFix`/`X-Forwarded-Prefix` corrige uniquement les URLs *générées* par Flask (redirections, `url_for`), pas le chemin *reçu*. **Corrigé** : `proxy_pass http://192.168.1.109:8088/;` (slash final, retire le préfixe `/fetcher/` avant transmission) — les deux mécanismes sont nécessaires ensemble, pas l'un ou l'autre. Navidrome, lui, n'avait pas ce problème (il connaît nativement son sous-chemin via `ND_BASEURL`, voir Phase 2) : c'est précisément pour ça que les deux `location{}` de ce fichier ne se ressemblent pas.
- **Vérifié réellement de bout en bout, via `https://music-jvince.duckdns.org` (edge, Pi) → second serveur**, pas seulement en accès direct :
  - `/navidrome/` → `302` vers `/navidrome/app/` (page de connexion Navidrome).
  - `/fetcher/login` → `200` ; `POST /fetcher/login` → session, redirection vers `/fetcher/` (avec le bon préfixe) ; `/fetcher/` après connexion → `200`.
  - **Téléchargement réel soumis via ce chemin complet** (edge → fetcher sur le second serveur) : vidéo YouTube publique courte téléchargée, fichier déposé dans le volume `music` du second serveur, **indexé par Navidrome en quelques secondes** (watcher de fichiers, confirmé dans les logs réels du conteneur).
- **Compte fetcher créé pour la validation** : `julien` / mot de passe temporaire `ChangeMe123` — **à changer** (`docker compose exec fetcher python manage.py add julien` puis choisir un vrai mot de passe).
- **Pas encore fait** : validation visuelle réelle dans un navigateur (pas seulement `curl`) ; Phase 5 (configuration Symfonium) ; Phase 6 (décision sauvegarde).
